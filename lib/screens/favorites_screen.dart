...
class FavoritesScreen extends StatelessWidget {
  ...
  const FavoritesScreen({Key? key, required this.favoriteStates})
    : super(key: key);
      ...
      body: FutureBuilder<List<List<Map<String, dynamic>>>>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            final favoriteItems = allData
                .where(
                  (item) =>
                      favoriteStates['${item['table_name']}-${item['id']}']?['isFavorite'] ==
                      true,
                )
                .toList();
            ...
            return ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                final color =
                    favoriteStates[key]?['color'] as Color? ?? Colors.white;
  ...
  Future<List<Map<String, dynamic>>> fetchData(String tableName) async {
    final response = await supabase.from(tableName).select('*').withConverter((
      data,
    ) {
